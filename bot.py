...
def get_amount_keyboard():
    ...
    keyboard_buttons = [buttons[i: i+4] for i in range(0, len(buttons), 4)]
...
async def select_amount(callback: types.CallbackQuery, state: FSMContext):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='✅ Confirm & Pay', callback_data='confirm_payment')],
...
async def confirm_payment(callback: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(BuyTicketStates.confirming_txid)
...
@dp.message(BuyTicketStates.confirming_txid)
async def receive_txid(message: types. Message, state: FSMContext):
    ...
    try:
        ...
        if user.get('referred_by'):
            raffle_stats = Raffle.get_raffle_stats()
            if raffle_stats['tickets_sold'] >= 700:
        ...
        await message.answer('❌ Error processing your purchase. Please try again.')
...
async def raffle_info(callback: types. CallbackQuery):
    ...
    message_text = (
        ...
        f'💡 Get 10% commission from referrals!'
    )
